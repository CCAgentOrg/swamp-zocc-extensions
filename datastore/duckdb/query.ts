...
const QueryResultRow = z.record(z.string(), z.unknown());
  ...
  methods: {
    list_tables: {
      ...
      execute: async (
        ...
        const tableMap = new Map<string, z.infer<typeof TableSchema>>();
    ...
    summarize: {
      ...
      execute: async (
        ...
        const tables: z.infer<typeof SummaryRow>[] = [];
        for (const row of rows) {
          const tableName = String(row["table_name"] ?? "");
    ...
    import_data: {
      ...
      execute: async (
        ...
        switch (fmt) {
          case "parquet": {
            if (args.create_table) {
              sql = `CREATE TABLE IF NOT EXISTS "${
                escId(args.table)
              }" AS SELECT * FROM read_parquet('${args.source}')`;
            } else {
              sql = `INSERT INTO "${
                escId(args.table)
              }" SELECT * FROM read_parquet('${args.source}')`;
            }
            break;
          }
          case "json": {
            if (args.create_table) {
              sql = `CREATE TABLE IF NOT EXISTS "${
                escId(args.table)
              }" AS SELECT * FROM read_json_auto('${args.source}')`;
            } else {
              sql = `INSERT INTO "${
                escId(args.table)
              }" SELECT * FROM read_json_auto('${args.source}')`;
            }
            break;
          }
          case "ndjson": {
            if (args.create_table) {
              sql = `CREATE TABLE IF NOT EXISTS "${
                escId(args.table)
              }" AS SELECT * FROM read_ndjson('${args.source}')`;
            } else {
              sql = `INSERT INTO "${
                escId(args.table)
              }" SELECT * FROM read_ndjson('${args.source}')`;
            }
            break;
          }
          case "csv":
          default: {
            ...
            if (args.create_table) {
              sql = `CREATE TABLE IF NOT EXISTS "${
                escId(args.table)
              }" AS SELECT * FROM read_csv_auto('${args.source}', header=${args.header}, delim='${delim}')`;
            } else {
              sql = `INSERT INTO "${
                escId(args.table)
              }" SELECT * FROM read_csv_auto('${args.source}', header=${args.header}, delim='${delim}')`;
            }
            break;
          }
        }
        ...
        const countResult = await runDuckDB(
          args.database,
          `SELECT COUNT(*)::VARCHAR AS cnt FROM "${escId(args.table)}"`,
        );
        if (countResult.success) {
          const countRows = parseCSV(countResult.stdout);
